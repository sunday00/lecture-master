...
class SoundedBall extends CircleComponent
  ...
  Map<String, SoundedBall> collision = <String, SoundedBall>{};
  ...
  void update(double dt) {
    ...
    for (var other in collision.entries) {
      SoundedBall otherObj = other.value;
  ...
  void onCollision(Set<Vector2> intersectionPoints, PositionComponent other) {
    ...
    if (other is SoundedBall) {
      if (!collision.containsKey(other.hashCode.toString())) {
        ...
        if (observedCollisions.contains(collisionKey)) {
          observedCollisions.remove(collisionKey);
        } else {
          observedCollisions.add(collisionKey);
