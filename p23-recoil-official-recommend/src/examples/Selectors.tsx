...
const eurSelector = selector<number>({
  ...
  get: ({get}) => {
    ...
    if(commissionEnabled) usd = removeCommission(usd, get(commissionAtom))

    return usd * exchangeRate
  },
  set: ({get, set}, newEurVal) => {
    let newUsdVal = (newEurVal as number) / exchangeRate

    const commissionEnabled = get(commissionEnabledAtom)

    if(commissionEnabled) newUsdVal = addCommission(newUsdVal, get(commissionAtom))
